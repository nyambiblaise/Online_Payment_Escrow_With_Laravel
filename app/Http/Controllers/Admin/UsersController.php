...
class UsersController extends Controller
{
    ...
    public function escrowLog($id)
    {
        $user = User::findOrFail($id);
        $userid = $user->id;
        $escrowList = Escrow::where(function ($query) use ($user) {
                $query->where('creator_id', $user->id)
                    ->orWhere('joiner_id', $user->id);
            })->latest()->paginate(config('basic.paginate'));
        $title = "Escrow ".$user->username;
        return view('admin.users.escrow-log', compact('user', 'userid', 'escrowList','title'));
    }
